...
class TelemetryClient:
    ...
    def start_embedding(self, input_texts: List[str], run_id: UUID, parent_run_id: Optional[UUID] = None, **attributes):
    ...
    def stop_embedding(self, run_id: UUID, embeddings: List[List[float]], **attributes) -> EmbeddingInvocation:
        """Stop an embedding invocation with results."""
        with self._lock:
            invocation = self._embedding_registry.pop(run_id)
        invocation.end_time = time.time()
        invocation.embeddings = embeddings
        invocation.attributes.update(attributes)
