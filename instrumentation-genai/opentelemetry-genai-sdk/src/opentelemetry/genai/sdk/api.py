...
class TelemetryClient:
    ...
    def start_embedding(self, run_id: UUID, gen_ai_system : str, model_name : str, parent_run_id: Optional[UUID] = None, **attributes):
    ...
    def fail_embedding(self, run_id: UUID, error: Error, **attributes) -> EmbeddingInvocation:
        """Fail an embedding invocation with error."""
        with self._lock:
            invocation = self._embedding_registry.pop(run_id)
        invocation.end_time = time.time()
        invocation.attributes.update(**attributes)
        self._exporter.error_embedding(error, invocation)
        return invocation
