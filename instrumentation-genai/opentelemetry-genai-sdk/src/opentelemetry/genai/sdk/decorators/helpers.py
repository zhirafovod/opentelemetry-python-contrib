...
def _find_llm_instance(args, kwargs):
    ...
    if not llm_instance:
        for key, value in kwargs.items():
            if key.lower() in ['llm', 'model', 'client'] and _is_llm_instance(value):
                llm_instance = value
                break
    ...
    if not llm_instance:
        try:
            import sys
            frame = sys._getframe(2)  # Go up 2 frames to get to the decorated function
            ...
            for var_name, var_value in local_vars.items():
                if _is_llm_instance(var_value):
                    ...
                    break
                elif hasattr(var_value, 'llm') and _is_llm_instance(var_value.llm):
...
def _is_llm_instance(obj):
    ...
    llm_patterns = [
        'ChatOpenAI', 'OpenAI', 'AzureOpenAI', 'AzureChatOpenAI',
    ...
    return any(pattern in class_name for pattern in llm_patterns) or 'langchain' in module_name.lower()
...
def _extract_llm_config_attributes(llm_instance, attributes):
    ...
    try:
        ...
        # Extract max_tokens
        if hasattr(llm_instance, 'max_tokens') and llm_instance.max_tokens is not None:
        ...
        if hasattr(llm_instance, 'frequency_penalty') and llm_instance.frequency_penalty is not None:
            attributes['request_frequency_penalty'] = float(llm_instance.frequency_penalty)
        
        # Extract presence_penalty
        if hasattr(llm_instance, 'presence_penalty') and llm_instance.presence_penalty is not None:
            attributes['request_presence_penalty'] = float(llm_instance.presence_penalty)
        ...
        elif hasattr(llm_instance, 'stop_sequences') and llm_instance.stop_sequences is not None:
