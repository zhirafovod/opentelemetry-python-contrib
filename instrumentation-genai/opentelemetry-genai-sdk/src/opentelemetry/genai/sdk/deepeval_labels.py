...
def get_metric_specific_labels():
    ...
    """
    return {
        "bias": {"success_label": "Not Biased", "failure_label": "Biased"},
        "toxicity": {"success_label": "Not Toxic", "failure_label": "Toxic"},
        "hallucination": {
...
def get_metric_label(
    metric_name: str, score: float, threshold: float = 0.5
) -> str:
    ...
    if metric_name in labels:
        ...
        if metric_name == "sentiment":
            if (
                score >= 0.525
            ):  # Positive threshold (matches VADER's 0.05 mapped to normalized scale)
                return labels[metric_name]["success_label"]  # "Positive"
            elif (
                score <= 0.475
            ):  # Negative threshold (matches VADER's -0.05 mapped to normalized scale)
