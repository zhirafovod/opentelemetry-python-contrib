...
from opentelemetry import _events, metrics, trace
...
class DeepEvalEvaluator(Evaluator):
    ...
    def _do_telemetry(self, invocation: LLMInvocation, eval_results):
        ...
        if isinstance(eval_results, dict) and isinstance(
            eval_results.get("error"), dict
        ):
        ...
        for metric_name, metric_data in eval_results.items():
            if metric_name != "error" and isinstance(metric_data, dict):
                ...
                response_id = (
                    invocation.attributes.get("response_id")
                    if invocation.attributes
                    else None
                )
                ...
                metric_attrs = {
                    "gen_ai.operation.name": "evaluation",
                    "gen_ai.request.model": invocation.attributes.get(
                        "request_model"
                    ),
                    "gen_ai.provider.name": invocation.attributes.get(
                        "provider_name"
                    ),
                    "gen_ai.evaluation.score.label": label_val,
                }
                ...
                if name_lower == "relevance":
                    self._metric_relevance.record(
                        score_val,
                        attributes={
                            k: v
                            for k, v in metric_attrs.items()
                            if v is not None
                        },
                    )
                elif name_lower == "hallucination":
                    self._metric_hallucination.record(
                        score_val,
                        attributes={
                            k: v
                            for k, v in metric_attrs.items()
