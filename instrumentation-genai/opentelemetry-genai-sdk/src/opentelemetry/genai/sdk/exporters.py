...
from .types import LLMInvocation, ToolInvocation, EmbeddingInvocation
...
def _embedding_input_to_event(embedding_input)-> Optional[Event]:
    """
    Convert embedding input to an Event for telemetry.
    
    Args:
        embedding_input: The input text(s) for embedding
    """
    ...
    body = {
        ...
        "role": "embedding", # TODO: revisit this field, not sure what role is
    }
    ...
    return Event(
        name="gen_ai.embedding.query",
        attributes=attributes,
        body=body,
    )


def _embedding_input_to_log_record(embedding_input)-> Optional[LogRecord]:
    """
    Convert embedding input to a LogRecord for telemetry.
    ...
    return LogRecord(
            event_name="gen_ai.embedding.query",
            attributes=attributes,
            body=body,
        )

def _embedding_output_to_event(embedding_output)-> Optional[Event]:
    """
    Convert embedding output to an Event for telemetry.
    
    Args:
        embedding_output: The output embeddings (vector data)
    """
    ...
    return Event(
        name="gen_ai.embedding.response",
        attributes=attributes,
        body=body,
    )


def _embedding_output_to_log_record(embedding_output)-> Optional[LogRecord]:
    """
    Convert embedding output to a LogRecord for telemetry.
    ...
    return LogRecord(
            event_name="gen_ai.embedding.response",
            attributes=attributes,
            body=body,
        )
...
class BaseExporter:
    ...
    def init_embedding(self, invocation: EmbeddingInvocation):
        raise NotImplementedError
    ...
    # TODO: generated by windsurf, need verification
    def export_embedding(self, invocation: EmbeddingInvocation):
        raise NotImplementedError
    ...
    # TODO: generated by windsurf, need verification
    def error_embedding(self, error: Error, invocation: EmbeddingInvocation):
        raise NotImplementedError
...
class SpanMetricEventExporter(BaseExporter):
    ...
    def export_embedding(self, invocation: EmbeddingInvocation):
        operation_name = invocation.attributes.get(GenAI.GEN_AI_OPERATION_NAME)
        request_model = invocation.attributes.get(GenAI.GEN_AI_REQUEST_MODEL)
        span = self._start_span(
            name=f"{operation_name} {request_model}",
            kind=SpanKind.CLIENT,
            parent_run_id=invocation.parent_run_id,
        )
        ...
        with use_span(
            ...
            # Emit events and log records for embedding input
            if invocation.input is not None:

                # TODO: remove deprecated event logging and its initialization and use below logger instead
                self._event_logger.emit(_embedding_input_to_event(
                    embedding_input=invocation.input,
                ))
                
                # # TODO: logger is not emitting event name, fix it
                self._logger.emit(_embedding_input_to_log_record(
                    embedding_input=invocation.input,
                ))
            ...
            # Set embedding dimensions if available
            ...
            if request_model:
                metric_attributes[GenAI.GEN_AI_REQUEST_MODEL] = request_model
            self._duration_histogram.record(elapsed, attributes=metric_attributes)
...
class SpanMetricExporter(BaseExporter):
    ...
    def export_llm(self, invocation: LLMInvocation):
        ...
        with use_span(
            ...
            # Set server address and port if available
            server_address = invocation.attributes.get("server.address")
            if server_address:
                span.set_attribute("server.address", server_address)
            
            server_port = invocation.attributes.get("server.port")
            if server_port:
    ...
    def export_embedding(self, invocation: EmbeddingInvocation):
        ...
        with use_span(
            ...
            # Set embedding dimensions if available
            if invocation.dimension_count > 0:
                span.set_attribute("gen_ai.embedding.dimension", invocation.dimension_count)
