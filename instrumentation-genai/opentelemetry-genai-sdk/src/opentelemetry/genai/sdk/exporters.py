...
class SpanMetricEventExporter(BaseExporter):
    ...
    def export_embedding(self, invocation: EmbeddingInvocation):
        ...
        with use_span(
            ...
            framework = invocation.attributes.get("gen_ai.framework")
            if framework:
                span.set_attribute("gen_ai.framework", framework)
            ...
            # Store span context in invocation
            invocation.span_id = span.get_span_context().span_id
            invocation.trace_id = span.get_span_context().trace_id
            ...
            if provider_name:
                metric_attributes["gen_ai.provider.name"] = provider_name
            if request_model:
                metric_attributes[GenAI.GEN_AI_REQUEST_MODEL] = request_model
            if framework:
                metric_attributes["gen_ai.framework"] = framework
            self._duration_histogram.record(elapsed, attributes=metric_attributes)
    ...
    def error_embedding(self, error: Error, invocation: EmbeddingInvocation):
        ...
        with use_span(
            ...
            framework = attributes.get("gen_ai.framework")
            if framework:
                span.set_attribute("gen_ai.framework", framework)
