...
class SpanMetricEventExporter(BaseExporter):
    ...
    def export_embedding(self, invocation: EmbeddingInvocation):
        ...
        with use_span(
            ...
            span.set_attribute(GenAI.GEN_AI_OPERATION_NAME, "embedding")
            if request_model:
                span.set_attribute(GenAI.GEN_AI_REQUEST_MODEL, request_model)
            ...
            # Set input texts count
            if invocation.input_texts:
                span.set_attribute("gen_ai.embedding.input_count", len(invocation.input_texts))

            # Set embedding dimensions if available
            if invocation.embeddings and len(invocation.embeddings) > 0:
                span.set_attribute("gen_ai.embedding.dimension", len(invocation.embeddings[0]))
                span.set_attribute("gen_ai.embedding.output_count", len(invocation.embeddings))

            response_model = attributes.get("response_model_name")
            if response_model:
                span.set_attribute(GenAI.GEN_AI_RESPONSE_MODEL, response_model)

            # End the embedding span
            self._end_span(invocation.run_id)
            invocation.span_id = span_state.span.get_span_context().span_id
            invocation.trace_id = span_state.span.get_span_context().trace_id
            ...
            elapsed = invocation.end_time - invocation.start_time
            metric_attributes = {
                GenAI.GEN_AI_OPERATION_NAME: "embedding"
            }
            if provider_name:
