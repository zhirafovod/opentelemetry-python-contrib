...
def assert_exemplars(exemplars, total, parent_span):
    assert len(exemplars) >= 1
    assert exemplars[0].value >= total
...
def test_langchain_call(
    ...
    _human_message = {"content": messages[1].content}
...
def test_langchain_call_with_tools(
    ...
    choice_logs = [
        record
        for record in logs
        if record.log_record.attributes.get("event.name") == "gen_ai.choice"
    ]
