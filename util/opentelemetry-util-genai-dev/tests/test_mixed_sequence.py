...
from opentelemetry.util.genai.types import LLMInvocation, ToolCall, EmbeddingInvocation
...
def test_mixed_sequence_llm_tool_llm_embedding_parenting():
    ...
    tool = ToolCall(name="translate", id="t1", arguments={"text": "hola"}, provider="prov")
    handler.start_tool_call(tool)
    assert tool.span is not None
    # Same trace id indicates proper parenting; span ids must differ
    assert tool.span.get_span_context().trace_id == llm1.span.get_span_context().trace_id
    assert tool.span.get_span_context().span_id != llm1.span.get_span_context().span_id
    ...
    assert emb.span.get_span_context().trace_id == llm2.span.get_span_context().trace_id
