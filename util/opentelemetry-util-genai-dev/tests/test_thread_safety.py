...
from opentelemetry.util.genai.types import ToolCall, EmbeddingInvocation, LLMInvocation
...
def test_thread_safety_parallel_invocations():
    ...
    def run_embedding(i):
        try:
            inv = EmbeddingInvocation(request_model="embed-model", input_texts=[f"t{i}"])
