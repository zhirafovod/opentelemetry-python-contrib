...
_map_correlation = os.getenv(_MAP_CORRELATION_FLAG, "true") not in (
    "0",
    "false",
    "False",
)
...
class TraceloopTranslatorEmitter(EmitterMeta):
    ...
    def _translate_attributes(self, invocation: LLMInvocation) -> None:
        ...
        if not attrs:
            return
        legacy_keys_to_strip: list[
            str
        ] = []  # track keys we successfully mapped; only strip those
        ...
        for key in list(attrs.keys()):
            ...
            if not (
                is_prefixed
                or raw_key in _TRACELOOP_TO_SEMCONV
                or raw_key in _CONTENT_MAPPING
            ):
                continue
                ...
                if mapped:
                    ...
                    if mapped == "gen_ai.conversation.id":
                        if not _map_correlation:
                            ...
                            value, str
                        ) or not _CONVERSATION_ID_PATTERN.match(value):
                            mapped = None
                    if mapped:
                        if (
                            mapped == "gen_ai.prompt.template"
                            and not _is_content_capture_enabled()
                        ):
                            # Skip sensitive template unless opted-in
                            mapped = None
                        elif mapped == "gen_ai.prompt.template" and isinstance(
                            value, str
                        ):
                        ...
                        if mapped:
                            ...
                            if span is not None and mapped not in getattr(
                                span, "attributes", {}
                            ):
                                try:
        ...
        if (
            ...
            if input_messages:
                try:
                    ...
                    if (
                        span is not None
                        and "gen_ai.input.messages"
                        not in getattr(span, "attributes", {})
                    ):
                        try:
                            span.set_attribute(
                                "gen_ai.input.messages", serialized
                            )
                        except Exception:  # pragma: no cover
                            pass
                except Exception:
                    ...
                    if (
                        ...
                        try:
                            span.set_attribute(
                                "gen_ai.input.messages", fallback
                            )
                        except Exception:  # pragma: no cover
                            pass
            ...
            if output_messages:
                try:
                    ...
                    if (
                        ...
                        try:
                            span.set_attribute(
                                "gen_ai.output.messages", serialized
                            )
                        except Exception:  # pragma: no cover
                            pass
                except Exception:
                    ...
                    if (
                        ...
                        try:
                            span.set_attribute(
                                "gen_ai.output.messages", fallback
                            )
                        except Exception:  # pragma: no cover
                            pass
