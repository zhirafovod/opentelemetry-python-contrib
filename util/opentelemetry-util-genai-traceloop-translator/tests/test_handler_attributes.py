...
def test_handler_sets_custom_attributes():
    ...
    target_span = None
    for span in spans:
        if span.attributes and span.attributes.get("gen_ai.workflow.name") == "test_workflow":
            target_span = span
            break
    
    assert target_span is not None, "Should find a span with gen_ai.workflow.name"
    
    # Check if custom attributes are present
    assert target_span.attributes.get("gen_ai.workflow.name") == "test_workflow"
