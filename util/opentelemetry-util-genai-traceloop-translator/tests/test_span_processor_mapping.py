...
def test_traceloop_span_processor_maps_attributes():
    ...
    assert len(spans) == 2, (
        f"Expected 2 spans (original + synthetic), got {len(spans)}"
    )
    ...
    original_spans = [
        s
        for s in spans
        if s.attributes and "traceloop.workflow.name" in s.attributes
    ]
    assert len(original_spans) == 1, (
        "Should have exactly 1 original span with traceloop.* attributes"
    )
    ...
    for span in spans:
        if (
            span.attributes
            and span.attributes.get("gen_ai.workflow.name") == "flowA"
        ):
            transformed_span = span
            break

    assert transformed_span is not None, (
        "Should find a span with transformed gen_ai.* attributes"
    )
    ...
    assert (
        transformed_span.attributes.get("gen_ai.workflow.path")
        == "flowA/AgentA/step1"
    )
